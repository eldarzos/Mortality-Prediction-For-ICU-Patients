...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train/Validation/Test splitter for a specific time window.')
    parser.add_argument('root_dir',
                        type=str,
                        help='Directory containing subject folders (output of 4_truncate_timeseries.py).')
    parser.add_argument('-t', '--t-hours',
                        type=int, required=True,
                        help='Time window (e.g., 24 or 48) for which to split data.')
    parser.add_argument('-s', '--seed',
                        type=int, default=0,
                        help='Random seed for reproducibility (default: 0).')
    parser.add_argument('--test-size',
                        type=float, default=0.1,
                        help='Proportion of data to use for the test set (default: 0.1).')
    parser.add_argument('--valid-size',
                        type=int, default=1000,
                        help='Absolute number of samples to use for the validation set (default: 1000).')
    args = parser.parse_args()

    split_train_test(args.root_dir, args.t_hours, args.seed, args.test_size, args.valid_size)
