...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate value dictionary from training subject data for a specific time window.')
    parser.add_argument('root_dir',
                        type=str,
                        help='Directory containing subject folders and the "splits" subdirectory.')
    parser.add_argument('-t', '--t-hours',
                        type=int, required=True,
                        help='Time window (e.g., 24 or 48) for which the dictionary is generated.')
    parser.add_argument('-s', '--seed',
                        type=int, default=0,
                        help='Seed used for the data split (default: 0).')
    args = parser.parse_args()

    generate_value_dict(args.root_dir, args.t_hours, args.seed)
