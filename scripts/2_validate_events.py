...
def main(args):
    ...
    subjects = sorted([d for d in subdirectories if is_subject_folder(d) and \
                       os.path.isdir(os.path.join(args.subjects_root_path, d))])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Validate events linking to stays.csv, ensuring HADM_ID and ICUSTAY_ID consistency.")
    parser.add_argument(
        'subjects_root_path',
        type=str, help='Directory containing subject subdirectories (output of 1_subject_events.py).')
    args = parser.parse_args()
    print(f"Validating events in: {args.subjects_root_path}")
