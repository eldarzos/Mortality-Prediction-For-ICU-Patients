...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Create final NumPy arrays for model training.')
    parser.add_argument('root_dir',
                        type=str,
                        help='Root directory containing subject folders and "dictionaries".')
    parser.add_argument('-t', '--t-hours',
                        type=int, required=True,
                        help='Time window (e.g., 24 or 48) for the arrays.')
    parser.add_argument('-n', '--n-bins',
                        type=int, default=10,
                        help='Number of bins used during quantization (default: 10).')
    parser.add_argument('-s', '--seed',
                        type=int, default=0,
                        help='Seed used for the data split (default: 0).')
    parser.add_argument('--max-len',
                        type=int, default=10000,
                        help='Maximum sequence length for padding/truncation (default: 10000).')
    args = parser.parse_args()

    create_arrays(args.root_dir, args.t_hours, args.n_bins, args.seed, args.max_len)
